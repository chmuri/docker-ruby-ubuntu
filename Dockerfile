...
RUN wget https://cache.ruby-lang.org/pub/ruby/1.9/ruby-1.9.3-p551.tar.gz \
    ...
    && cd ruby-1.9.3-p551 \
    && ./configure --prefix=/usr/local/ruby-1.9.3 --with-openssl-dir=$HOME/.openssl/openssl-1.1.1g  \
    && make \
    && make install
...
COPY --form builder $HOME/.openssl/openssl-1.1.1g  $HOME/.openssl/openssl-1.1.1g 
